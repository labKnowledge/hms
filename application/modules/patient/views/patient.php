        ...
        <div class="modal-content">
            ...
            <div class="modal-body row">
                ...
                <option value="HCC">HCC</option>
                    ...
                    <div class="form-group col-md-6">
    <button id="openCameraButton" class="btn btn-info">Open Camera</button>
</div>
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getUserMedia({ video: true })
                .then(function (stream) {
                    // Successfully opened the camera, you can now do something with the stream
                    var video = document.createElement('video');
                    video.srcObject = stream;
                    video.play();
                })
                .catch(function (error) {
                    // Handle errors if the user denies camera access or any other issues
                    console.error('Error accessing camera:', error);
                });
        } else {
            alert('webcam not compatible with the system requirements. click OK to submit.');
        }
    }

    // Add a click event listener to the "Open Camera" button
...
<script>
    ...
    $(document).ready(function () {
        var table = $('#editable-sample').DataTable({
            ...
            columnDefs: [
            {
                targets: 4, // Column index (0-based) where options are present
                orderable: false
            }
            ],
            //   dom: 'lfrBtip',
