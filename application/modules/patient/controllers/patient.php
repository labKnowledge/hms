...
class Patient extends MX_Controller {
    ...
    public function addNew() {
        ...
        $address = $this->input->post('address').' - '.$this->input->post('location');
        ...
        $this->form_validation->set_rules('address', 'Village', 'trim|required|min_length[2]|max_length[500]|xss_clean');
...
// Validating Deposited Amount Field
    ...
    function addMedicalHistory() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->ion_auth->in_group('Doctor')) {
                $current_user_name = $this->db->get_where('doctor', array('ion_user_id' => $current_user))->row()->name;
                $current_user_email = $this->db->get_where('doctor', array('ion_user_id' => $current_user))->row()->email;

            }
            if ($this->ion_auth->in_group('Nurse')) {
                $current_user_name = $this->db->get_where('nurse', array('ion_user_id' => $current_user))->row()->name;
                $current_user_email = $this->db->get_where('nurse', array('ion_user_id' => $current_user))->row()->email;
    ...
    function medicalHistory() {
        ...
        $data['rad'] = $this->lab_model->getRadByPatientId($id);
        ...
        $data['procedure_histories'] = $this->patient_model->getProcedureHistoryByPatientId($id);
        $data['vital_histories'] = $this->patient_model->getVitalHistoryByPatientId($id);
        ...
        foreach ($data['rad'] as $rad) {
            ...
            $timeline[$rad->date + 3] = '<div class="panel-body profile-activity" >
                                            <h5 class="pull-left"><span class="label pull-right r-activity">' . lang('procedure') . '</span></h5>
                                            <h5 class="pull-right">' . date('d-m-Y', $rad->date) . '</h5>
                                            <div class="activity blue">
                                                <span>
                                                    <i class="fa fa-flask"></i>
                                                </span>
                                                <div class="activity-desk">
                                                    <div class="panel col-md-6">
                                                        <div class="panel-body">
                                                            <div class="arrow"></div>
                                                            <i class=" fa fa-calendar"></i>
                                                            <h4>' . date('d-m-Y', $rad->date) . '</h4>
                                                            <p></p>
                                                             <i class=" fa fa-user-md"></i>
                                                                <h4>' . $lab_doctor . '</h4>
                                                                    <a class="btn btn-xs invoicebutton" title="Lab" style="color: #fff;" href="lab/invoice?id=' . $rad->id . '"><i class="fa fa-file-text"></i>' . lang('report') . '</a>
                                                        </div>
                                                    </div> 
                                                </div>
                                            </div>
    ...
    function getPatient() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getPatientBysearch($search, $orderBy, $orderDirection);
            } else {
                $data['patients'] = $this->patient_model->getPatient($orderBy, $orderDirection);
            }
        } else {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getPatientByLimitBySearch($limit, $start, $search, $orderBy, $orderDirection);
            } else {
                $data['patients'] = $this->patient_model->getPatientByLimit($limit, $start, $orderBy, $orderDirection);
            }
        }
        //  $data['patients'] = $this->patient_model->getPatient();
    ...
    function getCaseList() {
        $requestData = $_REQUEST;
        $case_type = $this->input->get('caseType');
        $start = $requestData['start'];
        $limit = $requestData['length'];
        $search = $this->input->post('search')['value'];
        ...
        if ($limit == -1) {
            ...
            if (!empty($search)) {
                $data['cases'] = $this->patient_model->getMedicalHistoryByLimitBySearch($limit, $start, $search,$case_type);
            } else {
                $data['cases'] = $this->patient_model->getMedicalHistoryByLimit($limit, $start,$case_type);
            }
        }
        //  $data['patients'] = $this->patient_model->getPatient();
        $current_user = $this->ion_auth->get_user_id();
        foreach ($data['cases'] as $case) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                ...
                if($current_user==$case->authorID){
                    ...
                    $options1 ='<i class="fa fa-ban" style="font-size:30px;color:red"></i>';
                }
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                ...
                if($current_user==$case->authorID){
                    $options2 = '<a class="btn btn-info btn-xs btn_width delete_button" title="' . lang('delete') . '" href="patient/deleteCaseHistory?id=' . $case->id . '&redirect=case&caseType='.$case_type.'" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash-o"></i></a>';
                     }
                else{
                    $options2= '<i class="fa fa-ban" style="font-size:30px;color:red"></i>';
                }
            ...
            $info[] = array(
                ...
                $case->authorName.'('.$case->authorEmail.')',
                $patient_details,
                $case->title,
