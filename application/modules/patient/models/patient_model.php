...
class Patient_model extends CI_model {
    ...
    function insertPatient($data) {
       ...
       // Model
       public function getPatientsPaginated($offset, $limit) {
    ...
    function getPatient($orderBy = null, $orderDirection = 'asc') {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    function getPatientBySearch($search, $orderBy = null, $orderDirection = 'asc') {
        /*
        $this->db->order_by('id', 'desc');
    ...
    function getPatientByLimit($limit, $start, $orderBy = null, $orderDirection = 'asc') {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    
        if ($orderBy !== null) {
            $this->db->order_by($orderBy, $orderDirection);
        } else {
            $this->db->order_by('id', 'desc'); // Default order if not specified
        }
    ...
    function searchPatients($searchTerm) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->like('name', $searchTerm); // Filter by name using LIKE
        $this->db->order_by('id', 'desc'); // Order by id in descending order
        $this->db->limit(25); // Limit the results to 25 per search
        $query = $this->db->get('patient');
        return $query->result();
    }
    ...
    function getLatestPatients($limit) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->order_by('id', 'desc'); // Default order if not specified
        $this->db->limit($limit);
        $query = $this->db->get('patient');
    ...
    // Model
    public function getFilteredPatients($filter) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->like('name', $filter); // Adjust the column and condition as needed
    ...
    function getPatientByLimitBySearch($limit, $start, $search, $orderBy = null, $orderDirection = 'asc') {
        $this->db->limit($limit, $start);    
        $this->db->select('*')
            ->from('patient')
            ->where('hospital_id', $this->session->userdata('hospital_id'))
    ...
    function getMedicalHistoryByPatientId($id) {
        ...
        $this->db->where('case_type', 'Default');
        $query = $this->db->get('medical_history');
        return $query->result();
    }
    function getProcedureHistoryByPatientId($id) {
        $this->db->where('patient_id', $id);
        $this->db->where('case_type', 'Procedure');
        $query = $this->db->get('medical_history');
        return $query->result();
    }
    function getVitalHistoryByPatientId($id) {
        $this->db->where('patient_id', $id);
        $this->db->where('case_type', 'Nurse');
        $query = $this->db->get('medical_history');
        return $query->result();
    }
    ...
    function getMedicalHistoryByLimit($limit, $start, $case_Type) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    function getMedicalHistoryByLimitBySearch($limit, $start, $search, $case_Type) {
        $this->db->order_by('id', 'desc');
