...
public function index() {
    ...
    $data['labs'] = $this->lab_model->getLab('lab');

    if (!empty($id)) {
        $data['lab_single'] = $this->lab_model->getLabById($id);
    }

    $data['labtemplates'] = $this->lab_model->getTemplate();
    $data['radtemplates'] = $this->lab_model->getRadTemplate();
    $data['settings'] = $this->settings_model->getSettings();
    $data['categories'] = $this->lab_model->getLabCategory(); 
    $data['doctors'] = $this->doctor_model->getDoctor();

    // Preprocess selected patient for lab_single
    $selectedLabPatientId = !empty($data['lab_single']->patient) ? $data['lab_single']->patient : '';
    $selectedLabPatientName = '';
    foreach ($data['patients'] as $patient) {
        if ($patient->id == $selectedLabPatientId) {
            $selectedLabPatientName = $patient->name;
            break;
        }
    }
    $data['selectedLabPatientId'] = $selectedLabPatientId;
    $data['selectedLabPatientName'] = $selectedLabPatientName;
...
// Adding New Patient
    ...
    function editLab() {
        if ($this->ion_auth->in_group(array('admin', 'Doctor', 'Laboratorist', 'Nurse', 'Patient'))) {
            ...
            $selectedPatientId = !empty($data['lab']->patient) ? $data['lab']->patient : set_value('patient');
...
// Validating Price Field
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {
                ...
                $inserted_id = $this->db->insert_id();
                $this->session->set_flashdata('feedback', 'Added');
                redirect("lab/addTemplateView?id=" . "$inserted_id"."&type="."$templatetype");
            } else {
                $data = array(
                    'name' => $name,
                    'template' => $template,
...
// } 
    ...
    function getLab() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['labs'] = $this->lab_model->getLabBysearch($search,$type);
            } else {
                $data['labs'] = $this->lab_model->getLab($type);
            }
        } else {
            if (!empty($search)) {
                $data['labs'] = $this->lab_model->getLabByLimitBySearch($limit, $start, $search,$type);
            } else {
                $data['labs'] = $this->lab_model->getLabByLimit($limit, $start,$type);
            }
        }
        ...
        foreach ($data['labs'] as $lab) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Laboratorist', 'Doctor'))) {
                $options1 = ' <a class="btn btn-info btn-xs editbutton" title="' . lang('edit') . '" href="lab?id=' . $lab->id . '&type='.$lab->type.'"><i class="fa fa-edit"> </i> ' . lang('') . '</a>';
            } else {
                $options1 = '';
            }
