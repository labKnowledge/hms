...
class Finance_model extends CI_model {
    ...
    function getPaymentByDate($date_from, $date_to) {
        ...
        $this->db->join('patient', 'patient.id = medical_history.patient_id');
        $this->db->where('patient.hospital_id', $this->session->userdata('hospital_id'));
        $this->db->where('medical_history.date >=', $date_from);
        $this->db->where('medical_history.date <=', $date_to);
        $this->db->where('medical_history.case_type =', $case_type);
        $patients = $this->db->get()->result();
    
        $result = [];
        foreach ($patients as $patient) {
            // Check for prescription and lab data for each patient and append accordingly
            $patientData = (array)$patient;
            $patientData['prescription'] = $this->checkPrescriptionForPatient($patient->id, $date_from, $date_to);
            $patientData['lab'] = $this->checkLabForPatient($patient->id, $date_from, $date_to);
    ...
    function checkPrescriptionForPatient($patientId, $date_from, $date_to) {
        $this->db->select('*');
    ...
    function checkLabForPatient($patientId, $date_from, $date_to) {
        $this->db->select('*');
        $this->db->from('lab');
        $this->db->where('patient', $patientId);
        $this->db->where('date >=', $date_from);
...
// function getPaymentByDate($date_from, $date_to) {
//     $this->db->select('patient.id, patient.name, medical_history.*, lab.*, prescription.*');
//     $this->db->from('patient');
//     $this->db->where('patient.hospital_id', $this->session->userdata('hospital_id'));
//     $this->db->join('medical_history', 'patient.id = medical_history.patient_id', 'left');
//     $this->db->join('lab', 'patient.id = lab.patient', 'left');
//     $this->db->join('prescription', 'patient.id = prescription.patient', 'left');

//     // Use >= and <= for date range filtering
//     $this->db->where('medical_history.date >=', $date_from);
//     $this->db->where('medical_history.date <=', $date_to);

//     $this->db->or_where('lab.date >=', $date_from);
//     $this->db->or_where('lab.date <=', $date_to);

//     $this->db->or_where('prescription.date >=', $date_from);
//     $this->db->or_where('prescription.date <=', $date_to);

//     $this->db->where('case_type', 'Procedure');

//     // Limit the number of results if not all are needed
//     // $this->db->limit(10);
...
// }
 ...
 // function getPaymentByDate($date_from, $date_to) {
 //        $this->db->where('patient.hospital_id', $this->session->userdata('hospital_id'));
 //        $this->db->select('patient.*, lab.*, prescription.*');
 //        $this->db->from('patient');
 //        $this->db->join('lab', 'patient.id = lab.patient', 'left');
 //        $this->db->join('prescription', 'patient.id = prescription.patient', 'left');
 //        $this->db->where('lab.date >=', $date_from);
 //        $this->db->where('lab.date <=', $date_to);
 //        $this->db->where('prescription.date >=', $date_from);
 //        $this->db->where('prescription.date <=', $date_to);
 //        $query = $this->db->get();
 //        return $query->result();
 //    }
