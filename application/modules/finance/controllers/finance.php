...
// Adding New Patient
    ...
    function finalize() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist'))) {
            $id = $this->input->get('id');
            $this->finance_model->finalizePayment($id);
            $this->session->set_flashdata('feedback', 'Finalized and Locked');
            redirect('finance/payment');
        }
    }
    ...
    function getPayment() {
        ...
        foreach ($data['payments'] as $payment) {
            ...
            if ($this->ion_auth->in_group(array('admin'))) {
                ...
                if($payment->finalize_Status=='Active'){
                        $options4 = '<a class="btn btn-info btn-xs" title="Finalize" href="finance/finalize?id=' . $payment->id . '" onclick="return confirm(\'Are you sure you want to Finalize and Lock this Payment?\');"><i class="fa fa-check"></i> Finalize</a>';
                } 
             } 
